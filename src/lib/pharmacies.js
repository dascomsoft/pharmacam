...
/**
 * Recherche spécifique des pharmacies normales (non de garde)
 * @param {string} query - Terme de recherche
 * @param {string} city - Ville (Yaoundé/Douala)
 * @param {object} userLocation - Position GPS {lat, lng}
 * @returns {Array} Liste des pharmacies normales
 */
export function searchNormalPharmacies(query, city = null, userLocation = null) {
  console.log('🏪 Recherche pharmacies normales pour:', { query, city });
  
  // Filtrer pour exclure les pharmacies de garde
  let normalPharmacies = pharmaciesData.filter(pharmacy => 
    !pharmacy.garde_24h && pharmacy.type !== 'garde'
  );
  
  // Si query contient "pharmacies" ou "normales", on retourne toutes les pharmacies normales
  const normalizedQuery = query.toLowerCase().trim();
  const isGeneralSearch = normalizedQuery.includes('pharmacie') || 
                         normalizedQuery.includes('normale') || 
                         normalizedQuery === 'pharmacies';
  
  if (isGeneralSearch) {
    // Retourner toutes les pharmacies normales de la ville
    let results = city ? 
      normalPharmacies.filter(p => p.ville === city) :
      normalPharmacies;
    
    // Trier par ville et nom
    ...
    let results = normalPharmacies.filter(pharmacy => {
      const matchesName = pharmacy.nom.toLowerCase().includes(normalizedQuery);
      const matchesQuartier = pharmacy.quartier?.toLowerCase().includes(normalizedQuery) || false;
      const matchesCity = city ? pharmacy.ville === city : true;
      
      return (matchesName || matchesQuartier) && matchesCity;
    });
...
export default {
  ...
  searchNormal: searchNormalPharmacies, // AJOUTÉ
  ...
  databaseInfo: {
    ...
    features: ['recherche_locale', 'pharmacies_normales', 'geolocalisation', 'ia_groq']
  }
};